...
int freezeDuration = 5000; // 5 Zseconds in milliseconds
int freezeStartTime = -5000; // Track when the freeze started
...
void serialEvent(Serial port) {
  ...
  try {
    ...
    // If currently frozen, do not update targetZoom
    if (millis() - freezeStartTime < freezeDuration) {
      return;
    }

    // Trigger a new 10-second freeze if threshold is exceeded
    if (fsrRaw > fsrThreshold) {
      freezeStartTime = millis();
      println("Freeze triggered for 10 seconds at raw: " + fsrRaw);
      return; 
    }

    // Normal zoom mapping when not frozen
