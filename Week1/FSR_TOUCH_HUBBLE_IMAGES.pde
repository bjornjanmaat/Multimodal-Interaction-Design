...
void loadHubbleImage() {
  try {
    // 1. Query the NASA Image Library for Hubble images (page size 100)
    // You can also randomize the page parameter: &page=1, &page=2, etc.
    int randomPage = int(random(1, 10));
    String apiUrl = "https://images-api.nasa.gov/search?q=hubble&media_type=image&page=" + randomPage;
    
    JSONObject response = loadJSONObject(apiUrl);
    JSONObject collection = response.getJSONObject("collection");
    JSONArray items = collection.getJSONArray("items");

    if (items.size() > 0) {
      // 2. Pick a random item from the results
      int randomIndex = int(random(items.size()));
      JSONObject selectedItem = items.getJSONObject(randomIndex);

      // 3. Extract metadata title
      JSONArray dataArray = selectedItem.getJSONArray("data");
      String title = dataArray.getJSONObject(0).getString("title");

      // 4. Extract image URL (preview/thumbnail link)
      JSONArray links = selectedItem.getJSONArray("links");
      String imgUrl = links.getJSONObject(0).getString("href");

      println("Downloading: " + imgUrl);

      // 5. Download the image over HTTP
      PImage loaded = loadImage(imgUrl);
    ...
    println("HTTP / Parsing Error: " + e.getMessage());
    //imgTitle = "Failed to load image. Press Space to retry.";
  } finally {
    isLoading = false;
  }
}
