...
struct ScaleView: View {
    ...
    @ObservedObject private var bridge = ProcessingBridge.shared
...
//                    .ignoresSafeArea()
                ...
                VStack(spacing: geometry.size.height * 0.06) {
                    ...
                    if !bridge.portName.isEmpty {
                        HStack(spacing: 8) {
                            Circle()
                                .fill(bridge.isTouchActive ? Color.green : Color.blue)
                            ...
                            Text(bridge.portName)
                                .font(.system(size: 12, weight: .bold, design: .monospaced))
                                .foregroundStyle(.primary)
                            
                            Text("• FSR: \(bridge.currentFsr)/1200")
                                .font(.system(size: 12, design: .monospaced))
                                .foregroundStyle(.secondary)
                            
                            Button(action: {
                                bridge.copyPortToClipboard()
                            }) {
                                HStack(spacing: 4) {
                                    Image(systemName: bridge.isCopied ? "checkmark" : "doc.on.doc")
                                    Text(bridge.isCopied ? "Copied" : "Copy")
                                }
